...
describe "when i send a get request to api-v1-deities" do
  it "I should recieve a 200 respose with all the deities" do
    ...
    get 'api/v1/deities.json'
    
    json_deduced = JSON.parse(response.body)
    
    expect(response).to have_http_status(200)
    expect(json_deduced.count).to eq(10)
    expect(json_deduced.first.count).to eq(8)
    expect(json_deduced.first["name"]).to eq(deity_1.name)
    expect(json.deduced.first["description"].to eq(deity_1.description))
    expect(json_deduced.last["name"]).to eq(deity_2.name)
    expect(json.deduced.last["description"].to eq(deity_2.description))
  end
  ...
  it "should receive one deity" do
    ...
    get 'api/v1/deities/1.json'
    
    json_deduced = JSON.parse(response.body)
    
    expect(response).to have_http_status(200)
    expect(json_deduced.count).to eq(8)
    expect(json_deduced["name"]).to eq(deity_1.name)
    expect(json.deduced["description"].to eq(deity_1.description))
  end
  ...
  it "should be able to retreive a deity's mythology" do
    ...
    get 'api/v1/deities/1/mythology.json'
    ...
    expect(json.deduced["origin_story"].to eq(mythology.description))
  end
  ...
  it "should be able to return a deity's events" do
    ...
    event_1 = Event.first
    event_2 = Event.last
    get 'api/v1/deities/1/events.json'
    ...
    expect(json.deduced.first["origin_story"].to eq(event_1.description))
    expect(json_deduced.last["title"]).to eq(event_2.name)
    expect(json.deduced.last["origin_story"].to eq(event_2.description))
  end
